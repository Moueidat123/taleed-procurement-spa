...
export default function App() {
  ...
  return <HashRouter><AppErrorBoundary><Suspense fallback={<div className="loading-state" role="status"><span className="loader"/>Loading your workspace…</div>}>{bootError?<DataControls recovery/>:<Routes>
    ...
    <Route element={<Guard/>}><Route path="/app" element={<Layout/>}><Route index element={<WorkspaceRedirect/>}/>
      ...
      <Route element={<Guard roles={['analyst','manager','admin']}/>}><Route path="portfolio" element={<Portfolio/>}/><Route path="organizations" element={<Organizations/>}/><Route path="organizations/:id" element={<OrganizationDetail/>}/><Route path="compare" element={<Compare/>}/><Route path="frameworks" element={<Frameworks/>}/></Route>
      <Route element={<Guard roles={['manager','admin']}/>}><Route path="cycles" element={<Cycles/>}/><Route path="access" element={<Access/>}/></Route>
